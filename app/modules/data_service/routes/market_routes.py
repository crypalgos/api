...
class OrderBookWSManager:
    ...
    async def connect(self, symbol: str, websocket: WebSocket):
        ...
        if symbol not in self.active_connections:
            ...
            self.listener_tasks[symbol] = asyncio.create_task(
                self._subscribe_zmq_feed(symbol)
            )

        self.active_connections[symbol].add(websocket)
        logger.info(
            f"Client connected to order book feed for {symbol}. Total: {len(self.active_connections[symbol])}"
        )
    ...
    def disconnect(self, symbol: str, websocket: WebSocket):
        if symbol in self.active_connections:
            ...
            logger.info(
                f"Client disconnected from order book feed for {symbol}. Remaining: {len(self.active_connections[symbol])}"
            )
...
async def get_orderbook_snapshot(
    symbol: str, depth: int = Query(20, enum=[10, 20, 50])
) -> JSONResponse:
    proj = orderbook_registry.get_projection("delta", symbol)
