...
class ResearchRunRepository(BaseRepository[ResearchRun]):
    ...
    async def get_runs_paginated(
        ...
        if parent_run_id:
            filters.append(ResearchRun.parent_run_id == parent_run_id)
        ...
        if sort_by == "created_at":
            ...
            stmt = stmt.order_by(
                ResearchRun.is_favorite.desc(), ResearchRun.updated_at.desc()
            )
        else:
    ...
    ) -> StrategyLatestResults | None:
        """Fetch the latest runs mapping for a strategy."""
        stmt = select(StrategyLatestResults).where(
            StrategyLatestResults.strategy_id == strategy_id
        )
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()

    async def update_latest_run(
        self, strategy_id: str, run_type: str, run_id: str
    ) -> None:
