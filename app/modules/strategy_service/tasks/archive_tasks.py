...
@celery_app.task(name="app.modules.strategy_service.tasks.archive_strategy_run_task", bind=True, max_retries=5, default_retry_delay=60)
...
async def _archive_async(self, run_id: str):
    ...
    if not db_events:
        logger.warning(f"No events found in DB for strategy run {run_id}; skipping archival.")
        return
    ...
    for ev in db_events:
        ...
        elif ev.event_type in ("OrderSubmittedEvent", "OrderAcceptedEvent", "OrderCancelledEvent"):
