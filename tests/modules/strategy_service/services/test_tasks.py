...
async def test_run_asynchronous_backtest_task_success(
    ...
    def get_side_effect(model_class, model_id):
        if model_class == Strategy:
            ...
            return ResearchRun(
                id=model_id, strategy_id="strat-123", type="BACKTEST", status="RUNNING"
            )
        return None
    ...
    mock_report = {
        ...
        "datasets": {"global_equity_curve": [[12345678, 10000.0], [12345679, 10500.0]]},
...
"""
    ...
    assert "Calling built-in function 'eval()' is strictly forbidden." in str(
        exc_info.value
    )
...
async def test_run_backtest_manifest_validation_failure(
    ...
    mock_report = {
        "metrics": {"global": {"net_profit": 500.0, "win_rate": 0.75}},
        "report": {
            "datasets": {"invalid_reference": {"dataset_id": "non_existent_dataset"}}
        },
        "datasets": {},
        "trades": [],
        "monthly": {},
