...
async def test_orderbook_projection_updates():
    inst = InstrumentId(
        exchange="delta", market_type=MarketType.PERPETUAL, symbol="BTCUSD"
    )
    ...
    changed = proj.apply_update(
        bids, asks, sequence=101, timestamp=int(time.time() * 1000)
    )
    ...
    changed_no_top = proj.apply_update(
        bids_no_change, [], sequence=102, timestamp=int(time.time() * 1000)
    )
    assert changed_no_top is False

    # 3. Apply updates deleting a level (size=0)
    delete_bid = [[Decimal("90000.0"), Decimal("0.0")]]
    changed_del = proj.apply_update(
        delete_bid, [], sequence=103, timestamp=int(time.time() * 1000)
    )
    assert changed_del is True

    snap_del = proj.get_snapshot(depth=10)
    assert snap_del.bids[0].price == Decimal(
        "89900.0"
    )  # Level 90000.0 deleted, top of book is 89900.0
...
async def test_orderbook_market_type_constraint():
    ...
    with pytest.raises(NotImplementedError) as exc:
        orderbook_registry.get_projection(
            exchange="delta", symbol="BTCUSD", market_type=MarketType.SPOT
        )
    ...
    with pytest.raises(NotImplementedError) as exc:
        orderbook_registry.get_projection(
            exchange="delta", symbol="BTCUSD", market_type=MarketType.OPTIONS
        )
    assert "Only perpetual futures are supported in v1.0" in str(exc.value)
